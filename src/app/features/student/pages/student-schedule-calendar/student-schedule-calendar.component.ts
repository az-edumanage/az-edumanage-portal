...
@Component({
    ...
    <section class="student-calendar-page">
      <header class="student-calendar-header">
        <div>
          ...
          <p>Your enrolled sessions displayed as a weekly timetable.</p>
        </div>
        <a class="student-calendar-button" routerLink="/student/schedule">
      ...
      @if (loading()) {
        ...
        @if (selectedCellDay() && selectedCellTime()) {
          <div class="student-calendar-dialog-backdrop" (click)="closeScheduleCell()">
            <section class="student-calendar-dialog" role="dialog" aria-modal="true" aria-labelledby="student-calendar-dialog-title" (click)="$event.stopPropagation()">
              <div class="student-calendar-dialog-head">
                <div>
                  <h2 id="student-calendar-dialog-title">Schedule</h2>
  ...
  `,
  styleUrl: './student-schedule-calendar.component.css',
...
export class StudentScheduleCalendarComponent implements OnInit {
  ...
  sessionsFor(day: string, time: string): StudentScheduleSession[] {
  ...
  formatSessionTimeRange(session: StudentScheduleSession): string {
  ...
  formatSessionDuration(session: StudentScheduleSession): string {
  ...
  colorClass(session: StudentScheduleSession): string {
