...
export class TenantGroupExamCreateDataService {
  ...
  loadGroupExamAssignment(groupId: string | null): Observable<GroupExamAssignment> {
    const selectedGroupId = this.requireId(groupId, 'Group is required');
    return this.http
      .get<GroupExamAssignment>(`${this.groupUrl(selectedGroupId)}/exam`)
      .pipe(catchError((error: HttpErrorResponse) => this.handleError(error, 'Unable to load group exam')));
  }

  saveGroupExamAssignment(groupId: string | null, payload: GroupExamAssignmentPayload): Observable<GroupExamAssignment> {
    const selectedGroupId = this.requireId(groupId, 'Group is required');
    return this.http
      .put<GroupExamAssignment>(`${this.groupUrl(selectedGroupId)}/exam`, payload)
      .pipe(catchError((error: HttpErrorResponse) => this.handleError(error, 'Unable to save group exam')));
  }

  loadPublishedExamOptions(stageId: string | null, gradeId: string | null, subjectId: string | null): Observable<PublishedGroupExamOption[]> {
    ...
    return this.http
      .get<PublishedGroupExamOption[]>(
        `${this.examsUrl}/${encodeURIComponent(selectedStageId)}/grades/${encodeURIComponent(selectedGradeId)}/subjects/${encodeURIComponent(selectedSubjectId)}`,
        { params },
      )
  ...
  loadExamQuestions(
    ...
    return this.http
      .get<GroupExamPreviewQuestion[]>(
        `${this.examsUrl}/${encodeURIComponent(selectedStageId)}/grades/${encodeURIComponent(selectedGradeId)}/subjects/${encodeURIComponent(selectedSubjectId)}/${encodeURIComponent(selectedExamId)}/questions`,
      )
