...
import { TenantExamsBasicEducationGradesComponent } from './tenant-exams-basic-education-grades.component';
...
describe('TenantExamsBasicEducationGradesComponent', () => {
  let fixture: ComponentFixture<TenantExamsBasicEducationGradesComponent>;

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [TenantExamsBasicEducationGradesComponent],
      providers: [
        provideRouter([]),
        {
    ...
    fixture = TestBed.createComponent(TenantExamsBasicEducationGradesComponent);
  ...
  it('links listed grades to the related subjects page', () => {
    const links = Array.from(fixture.nativeElement.querySelectorAll('a')).map((anchor) => (anchor as HTMLAnchorElement).pathname);

    expect(links).toContain('/tenant/exams/basic-education/stage-primary/grades/grade-one');
  });
  ...
  it('opens the related subjects page when a grade row is clicked', () => {
    ...
    expect(navigate).toHaveBeenCalledWith(['/tenant/exams/basic-education', 'stage-primary', 'grades', 'grade-one']);
  });
});
