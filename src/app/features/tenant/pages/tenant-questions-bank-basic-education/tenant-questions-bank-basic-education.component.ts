...
@Component({
  selector: 'app-tenant-questions-bank-basic-education',
              ...
              <tbody class="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-900">
                @for (stage of sortedStages(); track stage.id) {
                  <tr
                    ...
                    role="link"
                    tabindex="0"
                    [attr.aria-label]="'Open subjects for ' + stage.name"
                    (click)="openStageSubjects(stage.id)"
                    (keydown.enter)="openStageSubjects(stage.id)"
                    (keydown.space)="openStageSubjects(stage.id); $event.preventDefault()"
                  >
                    <td class="px-5 py-4">
                    ...
                    <td class="px-5 py-4 text-right">
                      <button type="button" (click)="openStageSubjects(stage.id); $event.stopPropagation()" class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 text-slate-600 transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 dark:border-slate-700 dark:text-slate-300 dark:hover:border-indigo-500/30 dark:hover:bg-indigo-500/10 dark:hover:text-indigo-300" [attr.aria-label]="'Open subjects for ' + stage.name">
...
export class TenantQuestionsBankBasicEducationComponent implements OnInit {
  ...
  openStageSubjects(stageId: string): void {
    void this.router.navigate(['/tenant/questions-bank/basic-education', stageId]);
  }
