...
@Component({
  selector: 'app-tenant-questions-bank-basic-education-grades',
      ...
      <section class="rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Select a grade to view its related subject question banks.</p>
          </div>
              ...
              <tbody class="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-900">
                @for (grade of stageGrades(); track grade.id) {
                  <tr
                    ...
                    role="link"
                    tabindex="0"
                    [attr.aria-label]="'Open subjects for ' + grade.name"
                    (click)="openGradeSubjects(grade.id)"
                    (keydown.enter)="openGradeSubjects(grade.id)"
                    (keydown.space)="openGradeSubjects(grade.id); $event.preventDefault()"
                  >
                    <td class="px-5 py-4">
                    ...
                    <td class="px-5 py-4 text-right">
                      <button type="button" (click)="openGradeSubjects(grade.id); $event.stopPropagation()" class="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 text-slate-600 transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 dark:border-slate-700 dark:text-slate-300 dark:hover:border-indigo-500/30 dark:hover:bg-indigo-500/10 dark:hover:text-indigo-300" [attr.aria-label]="'Open subjects for ' + grade.name">
...
export class TenantQuestionsBankBasicEducationGradesComponent implements OnInit {
  ...
  openGradeSubjects(gradeId: string): void {
    void this.router.navigate(['/tenant/questions-bank/basic-education', this.stageId(), 'grades', gradeId]);
  }
