...
const STORAGE_KEY = 'owner.subscription-statuses.v1';

@Injectable({ providedIn: 'root' })
export class OwnerSubscriptionStatusesDataService {
  ...
  upsertStatus(payload: {
    ...
    const next: TenantStatusItem = {
      id: `subscription_status_${crypto.randomUUID().slice(0, 8)}`,
      nameEn: payload.nameEn.trim(),
      nameAr: payload.nameAr.trim(),
      color: payload.color,
    };
  ...
  private loadSeed(): TenantStatusItem[] {
    const seeded: TenantStatusItem[] = [
      { id: 'sub_active', nameEn: 'Active', nameAr: 'نشط', color: '#16a34a' },
      { id: 'sub_trial', nameEn: 'Trial', nameAr: 'تجريبي', color: '#2563eb' },
      { id: 'sub_past_due', nameEn: 'Past Due', nameAr: 'متأخر', color: '#d97706' },
      { id: 'sub_suspended', nameEn: 'Suspended', nameAr: 'موقوف', color: '#dc2626' },
      { id: 'sub_cancelled', nameEn: 'Cancelled', nameAr: 'ملغي', color: '#64748b' },
    ];
